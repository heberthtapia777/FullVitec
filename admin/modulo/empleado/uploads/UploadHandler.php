 ...
 */
    ...
    if (!empty( $_REQUEST['idFile'] )){
        $idFile = $_REQUEST['idFile'];
    }else{
        $idFile = $_REQUEST['file'];
    }
    ...
    function getFotos(){
        global $db;
        global $idFile;
        $strQuery = "SELECT photo FROM empleado WHERE idEmpleado = '$idFile' AND photo != 'sin_imagen.jpg' ";
        $strQ = $db->Execute($strQuery);
        $array = array();
        while( $row = $strQ->FetchRow() ){
            array_push($array, $row['photo']);
        }
        return $array;
    }
    ...
    function deleteFotos(){
        global $db;
        global $idFile;
        //$strQuery = "DELETE FROM foto WHERE name = '".$foto."' ";
        $strQuery = "UPDATE empleado SET photo = 'sin_imagen.jpg', size = '' WHERE photo = '".$idFile."' ";
        $strQ = $db->Execute($strQuery);
    }

    function insertFotos($foto, $size, $type){
        global $db;
        $strQuery = "INSERT INTO auxImg (name, size, type) VALUE ('".$foto."', '".$size."', '".$type."')";
        $strQ = $db->Execute($strQuery);
    }
...
class UploadHandler
{
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error,
        ...
        $name = md5(date("Y-m-d-H:i:s").rand());
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    insertFotos($file->name, $file->size, $file->type);
                }
            } else {
