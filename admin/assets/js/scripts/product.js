...
function cargaEditor() {
	...
	tinymce.init({
		...
		mobile: {
			plugins:
				"print preview importcss searchreplace autolink autosave save directionality  visualblocks visualchars fullscreen image link media template codesample table charmap hr pagebreak nonbreaking anchor toc insertdatetime advlist lists wordcount textpattern noneditable help charmap emoticons"
		},

		menubar: " edit view format table tc help",
		toolbar:
			"undo redo | bold italic underline strikethrough | formatselect | alignleft aligncenter alignright alignjustify | outdent indent |  numlist bullist checklist | forecolor backcolor casechange permanentpen formatpainter removeformat | pagebreak | charmap emoticons | fullscreen  preview save print",
				...
				if (data.txtIdProduct == "") {
					if (data != 0) {
						Swal.fire({
							...
							img =
								'<a href="../../modulo/product/uploads/files/' +
								data.txtFoto +
								'" data-lightbox="image-' +
								data.lastId +
								'" ><img src="../../modulo/product/uploads/files/thumbnail/' +
								data.txtFoto +
								'"></a>';
							...
							button =
								'<button id="' +
								data.lastId +
								'" class="btn btn-warning btn-sm mr-1 mb-1" data-toggle="tooltip" title="Editar" onclick="cargarDataProduct(' +
								data.lastId +
								')"><i class="fas fa-pencil-alt"></i> </button>&nbsp;' +
								'<button class="btn btn-danger btn-sm mr-1 mb-1" data-toggle="tooltip" title="Eliminar" onclick="eliminarProduct(' +
								data.lastId +
...
function eliminarProduct(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.post("../../ajax/productAjax.php?op=delete", { id: id }, function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$("#txtDetails")
				...
				.addClass("focused");
			$("#txtPricePlata")
				.val(data.txtPricePlata)
				.closest("div")
				.addClass("focused");
			$("#txtPriceOro")
				.val(data.txtPriceOro)
				.closest("div")
				.addClass("focused");
			$("#txtEstado")
				.val(data.txtEstado)
